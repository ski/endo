...
**Tutorial 12 — The Mobile Wallet.** Now the mobile question makes sense. Bob's phone holds petnames for on-chain capabilities. It connects to an Agoric node via the chain's RPC endpoint. SES runs in the browser — `lockdown()` fires once at startup, before any component code runs, and everything after it is hardened. The UI is built with Lit and Web Components: each `<car-fob>` element holds its CapTP reference in a private field, exposes only the methods its shadow DOM needs, and is installed on the home screen as a PWA. The Web Component boundary rhymes with the capability boundary — the component's private state is the closure from tutorial 3, its tag name is a pet name, its shadow DOM is a confinement boundary for styles and layout. The synced pet store lives in IndexedDB for offline access. Revocation is enforced by the chain, not by Alice's daemon. The build pipeline is the same Lit + Vite + Cloudflare Workers stack you would recognise from any modern web app — the capability model is the new piece, not the infrastructure.
